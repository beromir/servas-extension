...
<div use:clickOutside={() => showDropdown = false} class="relative">
    ...
    {#if showDropdown}
        <ul class="absolute z-10 mt-1 max-h-40 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm dark:bg-gray-950 dark:ring-contrast">
            ...
            {:else}
                <li class="py-2 text-sm text-gray-700 text-center dark:text-gray-300">Nothing found</li>
            {/each}
        </ul>
    {/if}
</div>
